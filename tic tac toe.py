...
class TicTacToe:
    ...
    def inputPlayerLetter(self, player):
        ...
        while not (letter == 'X' or letter == 'O'):
            print('Does ' + player + ' want to be X or O?')
            letter = input().upper()
        return letter

    def whoGoesFirst(self, x, y):
...
while True:
    # Reset the board
    cur_board = TicTacToe()
    turn, firstplayer, secondplayer = cur_board.whoGoesFirst(nameOne, nameTwo)
    print(firstplayer + ' will go first.')
    c1 = cur_board.inputPlayerLetter(firstplayer)
    if c1 == 'X':
        c2 = 'O'
    else:
        c2 = 'X'
